...
def testDbTables(testConn, proConn, db):
    arr01 = []
    tablesDataList = proConn.ExecuteSql(
        f"SELECT * FROM `information_schema`.`TABLES` WHERE table_schema='{db}' AND table_comment='' ;")

    for tablesData in tablesDataList:
        print('tablesData===', tablesData)
        tablesTestData = testConn.ExecuteSql(
...
def testDbColumns(testConn, proConn, db):
    arr02 = []
    columnsDataList = proConn.ExecuteSql(
        f"SELECT * FROM `information_schema`.`COLUMNS` WHERE table_schema='{db}' AND column_comment='';")
    for columnsData in columnsDataList:
        print('columnsData', columnsData)
        columnsTestData = testConn.ExecuteSql(
